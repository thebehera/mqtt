...
class ComposablePlatformBuffer(
    val first: ReadBuffer,
    val second: ReadBuffer,
    val pool: BufferPool = BufferPool(UnlimitedMemoryLimit)
) : ReadBuffer {
    val firstInitialLimit = first.limit()
    val secondInitialLimit = second.limit()
        ...
        fun List<PlatformBuffer>.toComposableBuffer(): ReadBuffer {
            return when (size) {
                1 -> {
                    ...
                    ComposablePlatformBuffer(first(), subList(1, size).toComposableBuffer())
                }
            }
        }
    }
