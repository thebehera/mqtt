...
class TableMetadata<T : Any>(
    ...
    val nameAnnotationsMap = type.getAnnotationsIncludingSuperclass()
    ...
    val columns by lazy {
        ...
        members.forEach {
            columns[it.name] = TableColumn(this, it, customClasses, nativeClasses, spacer)
        }
        columns
    }
    ...
    private fun createTable(ifNotExists: Boolean): CharSequence {
        ...
        if (!hasForeignKey && !hasPrimaryKey) {
            sql.append(spacer)
            sql.append(" autogenerated_sql_id INTEGER NOT NULL PRIMARY KEY,$lineBreak")
        }
    ...
    fun delete(column: TableColumn) = "DELETE FROM '$tableName' WHERE ${column.name} = '\${${column.rawPropertyName}}';"
