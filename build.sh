...
if [ $TRAVIS_OS_NAME = "linux" ]; then
  ...
  ./gradlew --info check build -x lint allTests -x testDebugUnitTest -x testReleaseUnitTest --console=plain --max-workers=1 --build-cache -Dkotlin.colors.enabled=false
fi
